...
const DataFetchingComponent: React.FC<DataFetchingComponentProps> = ({ templateReportId }) => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${configApi.getString('backend.baseUrl')}/api/template-reporting/report/${templateReportId}`
        );
        if (!response.ok) {
        ...
        try {
          const scaffolderTask = (await scaffolderApi.getTask(jsonData.template_task_id))
          setScaffolderObj(scaffolderTask)
        } catch (error: any) {
          setScaffolderObj({status: "NOT FOUND"})
          
        }
        const ren = new MarkdownIt().renderer.render(JSON.parse(jsonData.report_rendered_content as string), {}, undefined)
    ...
    <Box>
        <Paper elevation={3}>
            <Grid container spacing={0}>
                ...
                <Grid item xs={12} sm={6} md={3}>
                    <Typography variant='h6'><b>Template Task ID:</b></Typography>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <Typography>{fullReport.template_task_id}</Typography>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <Typography variant='h6'><b>Created by:</b></Typography>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <Typography>{fullReport.created_by}</Typography>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <Typography variant='h6'><b>Template status:</b></Typography>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                <Typography style={{ color: statusColor }}>
{scaffolderObj.status}</Typography>
                </Grid>
